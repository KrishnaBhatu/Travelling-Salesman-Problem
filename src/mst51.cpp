 ...
 * param[in]: cost - Total cost of final tour
 **/
void write_output_to_file(std::vector<int> &final_path, int number_of_nodes,
                          int cost) {
  std::ofstream myfile;
  myfile.open("../output/mst51.output.tour");
  myfile << "NAME: "
         << "mst51.output.tour" << std::endl;
  myfile << "COMMENT : Tour for mst51.tsp (Length " << cost << ")" << std::endl;
  myfile << "TYPE : TOUR" << std::endl;
  myfile << "DIMENSION : " << number_of_nodes << std::endl;
  myfile << "TOUR SECTION" << std::endl;

  for (auto x : final_path)
    myfile << x << std::endl;
  myfile << "-1" << std::endl << "EOF";
...
/**
 * Brief: Reads the graph data from .tsp file
 * param[out]: vetrices - It's a map with the vertex ID and it's 2D pose
 * return: Number of vetrices in the graph given by the data (.tsp) file
 **/
int read_data_from_file(std::map<int, std::tuple<int, int>> &vertices) {
  std::ifstream data_file; // Object to fetch data from file
  data_file.open("../input/eil51.tsp");
  if (!data_file) {
  ...
  int number_of_nodes, key;
  std::string data;
  std::string search_string = "NODE_COORD_SECTION";
  std::string stop_string = "EOF";
  bool collect_data = false;
  std::vector<std::string> node_data;

  while (getline(data_file, data)) {
    if (data.find("DIMENSION :") != std::string::npos) {
      number_of_nodes =
          std::stoi(data.substr(12, -1)); // This gets us the number of vertices
    }

    if (collect_data &&
        data.find(stop_string, 0) ==
            std::string::npos) // Get Data till End of File
    {
      std::string word = "";
      ...
      words.push_back(
          word); // Words will have {key, x-coordinate, y-coordinate}
      std::tuple<int, int> point(std::stoi(words[1]), std::stoi(words[2]));
      key = std::stoi(words[0]);
      vertices[key] = point;
    }

    if (data.find(search_string, 0) != std::string::npos)
      collect_data = true;
  }
  data_file.close();
...
int main() {
  ...
  std::map<int, std::tuple<int, int>> vertices;
  int number_of_nodes = read_data_from_file(vertices);
  if (vertices.size() == 0)
    return -1;
  ...
  write_output_to_file(final_path, number_of_nodes, cost);
